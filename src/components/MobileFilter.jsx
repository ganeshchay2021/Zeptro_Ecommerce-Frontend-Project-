...
const MobileFilter = ({ openFilter, setOpenFilter,data, search, setSearch, category, setCategory, brand, setBrand, priceRange, setPriceRange, handleCategoryChange,
    handleBrandChange, handlePage, scrollToTOp, }) => {
    ...
    return (
        <>
            <div className='bg-gray-100 py-2 flex justify-between px-4 items-center mb-5 rounded-md md:hidden cursor-pointer' onClick={() => setOpenFilter(!openFilter)} >
                <h1 className='font-semibold text-xl'>Filters</h1>
                <FaFilter className='text-gray-800 ' />
            </div>

            {
                openFilter ? <div className='bg-gray-100 rounded-md p-4 h-max mb-5'>
                    <input type="text" placeholder='Search...' className='bg-white border p-2 rounded-md border-gray-400 w-full' onChange={(e) => setSearch(e.target.value)} value={search} onSubmit={() => setOpenFilter(false)} />
